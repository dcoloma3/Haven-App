            ...
            <div key={note.id} className={`bg-white border rounded-2xl p-4 ${note.is_pinned ? 'border-amber-200' : 'border-slate-200'}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  {isAdmin && (
                    <button onClick={() => togglePin(note)} className={`text-xs p-1 rounded transition-colors ${note.is_pinned ? 'text-amber-500 hover:text-amber-700' : 'text-slate-300 hover:text-slate-500'}`} title={note.is_pinned ? 'Unpin' : 'Pin'}>
                      📌
                    </button>
                  )}
                  {(isAdmin) && (
                    <button onClick={() => handleDelete(note.id)} className="text-slate-300 hover:text-red-400 transition-colors p-1">
