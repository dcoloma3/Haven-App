...
export default function ResidentProfile({ resident, onUpdate, onFaceSheet, generatingPDF }) {
    ...
    return (
      <>
        <div className="bg-white border border-slate-200 rounded-2xl p-6">
          {/* Card header — stacks on mobile, inline on desktop */}
          <div className="mb-5">
            ...
            {/* Secondary actions — icon buttons on mobile, text links on desktop */}
            <div className="flex items-center gap-3 mt-3">
              <button
                onClick={() => setShowEventForm(true)}
                className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-[#185FA5] transition-colors bg-slate-50 hover:bg-slate-100 rounded-lg px-3 py-1.5"
              >
              ...
              {onFaceSheet && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-[#185FA5] transition-colors bg-slate-50 hover:bg-slate-100 rounded-lg px-3 py-1.5 disabled:opacity-40"
                >
                  <svg className="w-3.5 h-3.5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <span>{generatingPDF ? 'Generating…' : 'Face Sheet'}</span>
                </button>
              )}
            </div>
          </div>
